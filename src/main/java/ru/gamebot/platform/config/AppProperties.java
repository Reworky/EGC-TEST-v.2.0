...
public class AppProperties {
    ...
    // Реквизиты для оплаты в пилоте "Донат по играм" (GemPurchaseService) — перевод идёт мимо бота,
    // вручную; задать реальную карту/СБП через переменную окружения APP_GEMPURCHASEPAYMENTDETAILS.
    private String gemPurchasePaymentDetails =
            "⚠️ Реквизиты для оплаты ещё не настроены — обратитесь к администратору клуба.";

    // TON-кошелёк клуба для оплаты донат-гемов в TON (2026-09-20, второй способ оплаты рядом с
    // рублями) — задать адрес через переменную окружения APP_GEMPURCHASETONWALLET. Пустая строка =
    // способ показывается, но с предупреждением "ещё не настроен" (тот же паттерн, что у рублей).
    private String gemPurchaseTonWallet = "";
}
