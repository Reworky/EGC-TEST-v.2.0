...
public class AppProperties {
    ...
    // TON-кошелёк клуба для оплаты донат-гемов в пилоте "Донат по играм" (GemPurchaseService, перевод
    // идёт мимо бота, вручную) — задать адрес через переменную окружения APP_GEMPURCHASETONWALLET.
    // Пустая строка = способ показывается, но с предупреждением "ещё не настроен". Рубли как способ
    // оплаты убраны 2026-09-20 (пользователь не принимает переводы на личную карту от игроков) — вместе
    // с ними удалено и gemPurchasePaymentDetails (было для рублей, стало мёртвым полем).
